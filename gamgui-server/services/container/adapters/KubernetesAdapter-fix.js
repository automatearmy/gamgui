...
class KubernetesAdapter extends ContainerService {
  ...
  /**
   * Initialize Kubernetes client using loadFromDefault for all environments.
   * @private
   */
  _initializeK8sClient() {
    try {
      ...
      // Always use loadFromDefault which works in both local and Cloud Run environments
      this.logger.info('Initializing Kubernetes client with loadFromDefault');
      try {
        this.kc.loadFromDefault();
        this.logger.info('Successfully initialized K8s client using loadFromDefault().');
      } catch (loadDefaultError) {
        this.logger.error('Failed to initialize K8s client using loadFromDefault():', loadDefaultError);
        throw new ContainerError('Failed to initialize K8s client from default config', { cause: loadDefaultError });
      }

      // Create the API clients using the configured KubeConfig
