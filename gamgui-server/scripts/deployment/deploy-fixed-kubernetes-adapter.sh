...
# Script para implantar a versão corrigida do KubernetesAdapter

set -e

echo "=== Implantando versão corrigida do KubernetesAdapter ==="
...
docker build --platform linux/amd64 -t gcr.io/$PROJECT/gamgui-server-image:fixed-init .
echo "✅ Imagem construída com sucesso."

# Enviar a imagem para o Container Registry
echo "Enviando a imagem para o Container Registry..."
docker push gcr.io/$PROJECT/gamgui-server-image:fixed-init
...
gcloud run services update gamgui-server \
  --region=us-central1 \
  --project=$PROJECT \
  --image=gcr.io/$PROJECT/gamgui-server-image:fixed-init \
  --update-env-vars="GKE_CLUSTER_NAME=$CLUSTER_NAME,GKE_CLUSTER_LOCATION=$CLUSTER_LOCATION,PROJECT_ID=$PROJECT"

echo "=== Implantação concluída ==="
echo "Agora o Cloud Run deve estar usando a versão corrigida do KubernetesAdapter."
