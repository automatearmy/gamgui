/**
 * Script to update require paths in diagnostic scripts
...
// Directory containing the diagnostic scripts
...
function updateRequirePaths(filePath) {
  try {
    ...
    content = content.replace(
      /require\(['"]([^./][^/].*)['"]\)/g, 
      "require(require(require('path').resolve(__dirname, '../../node_modules/path').resolve(__dirname, '../../node_modules/$1')))"
    );
...
// Main function
async function main() {
  log(GREEN, '=== Updating Require Paths ===');
  
  // Get all JavaScript files in the scripts directory
  const jsFiles = getJavaScriptFiles(scriptsDir);
  
  if (jsFiles.length === 0) {
    log(YELLOW, 'No JavaScript files found in the scripts directory.');
    return;
  }
