...
echo "4. Create GCS bucket for version tracking"
echo "5. Copy terraform.tfvars.example to terraform.tfvars"
...
# Create the GCS bucket for versions if it doesn't exist
echo -e "${BLUE}Creating GCS bucket for version tracking...${NC}"
if gsutil ls -b gs://${PROJECT_ID}-versions &>/dev/null; then
  echo -e "${YELLOW}Bucket gs://${PROJECT_ID}-versions already exists.${NC}"
else
  gsutil mb -c standard -l ${REGION} gs://${PROJECT_ID}-versions
  echo -e "${GREEN}Created bucket gs://${PROJECT_ID}-versions.${NC}"
fi
