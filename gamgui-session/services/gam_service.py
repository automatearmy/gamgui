...
class GamService:
    ...
    async def execute_command(
        self, command: str, command_id: Optional[str] = None
    ...
    def get_running_commands(self) -> Dict[str, Dict[str, Any]]:
        ...
        running = {}
        for command_id, process in self.running_processes.items():
            running[command_id] = {"pid": process.pid, "running": process.poll() is None}
        return running

    def is_gam_available(self) -> bool:
        """Check if GAM is available and executable"""
        try:
            result = subprocess.run([environment.GAM_PATH, "version"], capture_output=True, text=True, timeout=10)
